    ...
    <div className="xh:h-[100vh] fixed xl:static w-full h-full -left-full top-0 bg-gray-800 p-8 flex flex-col justify-between">
      <div>
        ...
        <ul>
          ...
          <li>
            <button onClick={() => setShowSubmenu(!showSubmenu)} className="flex items-center gap-4 py-2 px-4 rounded-lg hover:bg-black transition-colors" type="button">
              <RiBookOpenLine className='text-amber-500'/>Calidad Educativa<RiArrowDropRightLine className={`w-6 h-6 mt-1 ${showSubmenu && "rotate-90"} transition-all`} />
            </button>
          ...
          <li>
            <button className="flex items-center gap-4 py-2 px-4 rounded-lg hover:bg-black transition-colors hover:text-white" type='button'>
              <RiMenuSearchLine className='text-amber-500'/>Investigación<RiArrowDropRightLine className={`w-6 h-6 mt-1 ${showSubmenu && "rotate-90"} transition-all`} />
            </button>
            <ul className={`my-2 ${!showSubmenu && "hidden"}`}>
              <li>
